...
const objectID = new mongoose.Types.ObjectId(q)
  ...
  const { data, error } = useSWR(`http://localhost:1234/api/product?id=${objectID}`, fetcher);
  ...
  const new_product = useFormik({
    ...
    onSubmit: async (values, actions) => {
      try {
        await axios.put(`http://localhost:1234/api/product?id=${objectID}`, values);
  ...
  return (
    <>
      <div className="px-[60px] pt-3    lg:ml-[280px] sm:ml-0 h-screen w-full">
        ...
        <form
          onSubmit={new_product.handleSubmit}
            ...
            <div className=" px-3 mb-6 md:mb-0">
              ...
              <input
                className={
                  ...
                  new_product.touched.product_name
              ...
              <input
                className={
                  new_product.errors.descri && new_product.touched.descri
                    ? " bg-gray-50 border border-red-500 text-gray-900 text-sm rounded-lg focus:ring-red-600 focus:border-red-600 block w-full p-2.5 h-[130px]"
